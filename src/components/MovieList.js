...
import Slider from "react-slick";  //슬라이드 import
...
//비동기 요청  
  const fetchMovies = async ()=>{
    try{
      ...
      setError("에러 발생. 데이터 패치를 하지 못하였습니다.");
    }
  }
  useEffect(()=>{
    setIsLoading(true);
...
//arrow변경
  ...
  const settings = {
    ...
    slidesToScroll: 4,
    initialSlide: 0,
    nextArrow: <SampleNextArrow />,   //arrow버튼 변경 추가함
    prevArrow: <SamplePrevArrow />,   //arrow버튼 변경 추가함
    responsive: [
      {
        breakpoint: 960,
        settings: {
